...
const createService = async (serviceDetails, fileName) => {
  ...
  // Find the largest existing serviceId
  const maxServiceCount = await ServiceModel.findOne(
    {},
    { serviceId: 1 },
    { sort: { serviceId: -1 } }
  );

  // Calculate the next serviceId
  const nextServiceId = maxServiceCount ? maxServiceCount.serviceId + 1 : 1;
...
const deleteService = async (serviceId, deleteDetails) => {
  ...
  const serviceData = await ServiceModel.findOne({ serviceId }).select(
    "-_id isActive"
  );
