...
const createtFormForCareer = async (careerDetails, resumePdfPath) => {
    ...
    if(!firstName || !lastName || !email || !phone || !eligibleForEmployment || !employedByCityWide) {
        throw new Error(`Missing required field`);
    }

    if( 
        [firstName, lastName, email, phone, eligibleForEmployment, employedByCityWide].some((field) => field?.trim() === "")
    ) {
    ...
    const existingUsers = await CareerAndEmploymentModel.findOne({email: email});
    ...
    if (!validateResume(resumePdfPath)) {
        throw new Error(`Invalid resume file type. Only .doc, .docx, .pdf, .txt, .rtf are allowed.`);
    }
     // Uploading the pdf on cloudinary server
    const uploadedResumePdf = await fileUploadInCloudinary(resumePdfPath, "resumePdf");

    const careerAndEmploymentCount = await CareerAndEmploymentModel.countDocuments();

    const createCareerForm = new CareerAndEmploymentModel({
        careerAndEmploymentId: careerAndEmploymentCount + 1,
        firstName,
        lastName,
        email,
        phone,
